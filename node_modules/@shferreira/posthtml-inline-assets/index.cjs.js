  ...
  return function posthtmlInlineAssets(tree) {
    ...
    tree.walk(node => {
      ...
      Object.keys(transforms).forEach(type => {
        ...
        const origin = node && node.attrs && node.attrs['data-origin-src']; // resolved asset file path

        const assetpath = typeof transform.resolve === 'function' && typeof transform.transform === 'function' && (origin || transform.resolve(node));
        ...
        if (typeof assetpath === 'string') {
          ...
          promises.push(fs.readFile(from).then(buffer => {
            ...
            if (opts && opts.save_origin && node && node.attrs) node.attrs['data-origin-src'] = assetpath; // transform the node
